...
ZombieDefinition gZombieDefs[NUM_ZOMBIE_TYPES] = {  
    ...
    { ZOMBIE_PAIL,              REANIM_ZOMBIE,              4,      6,      1,      3000,   _S("BUCKETHEAD_ZOMBIE")},
    ...
    { ZOMBIE_DANCER,            REANIM_DANCER,              5,      3,     5,      1000,   _S("DANCING_ZOMBIE")},
    ...
    { ZOMBIE_BOSS,              REANIM_BOSS,                10,     NUM_LEVELS,     1,      0,      _S("BOSS")},
    ...
    { ZOMBIE_LETTER_HEAD,          REANIM_ZOMBIE,              3,      61,     6,     2000,   _S("LETTER_ZOMBIE")},
...
static ZombieType gBossZombieList[] = {  
    ...
    ZombieType::ZOMBIE_CONE_REPEATER_HEAD,
...
void Zombie::ZombieInitialize(int theRow, ZombieType theType, bool theVariant, Zombie* theParentZombie, int theFromWave)
{
    ...
    case ZombieType::ZOMBIE_CHERRY_BUNGEE:
    {
        mBodyHealth = 2000;
        mAnimFrames = 4;
        mAltitude = BUNGEE_ZOMBIE_HEIGHT + RandRangeInt(0, 150);
    ...
    case ZombieType::ZOMBIE_GIGA_FOOTBALL:
        ...
        case ZombieType::ZOMBIE_SUPER_ALLSTAR:
        mZombieRect = Rect(50, 0, 57, 115);
        ReanimShowPrefix("anim_hair", RENDER_GROUP_HIDDEN);
        mHelmType = HelmType::HELMTYPE_FOOTBALL;
        mHelmHealth = 2000;
        mBodyHealth = 2000;
        mZombiePhase = ZombiePhase::PHASE_ALLSTAR_RUNNING;
        mPhaseCounter = 30;
        mAnimTicksPerFrame = 6;
        mVariant = false;
        break;
    ...
    case ZombieType::ZOMBIE_SUPER_JACK:
    {
        mBodyHealth = 2500;
        mAnimTicksPerFrame = 6;
        ...
        mPhaseCounter = (int)((aDistance / mVelX) * ZOMBIE_LIMP_SPEED_FACTOR/1.3);
        mZombieAttackRect = Rect(20, 0, 50, 115);
    ...
    case ZombieType::ZOMBIE_RALLY:
    {
        ...
        Reanimation* aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
        Reanimation* aFlagReanim = mApp->AddReanimation(0.0f, 0.0f, 0, ReanimationType::REANIM_RALLY);
        aFlagReanim->PlayReanim("Zombie_rally", ReanimLoopType::REANIM_LOOP, 0, 15.0f);
        mSpecialHeadReanimID = mApp->ReanimationGetID(aFlagReanim);
        ReanimatorTrackInstance* aTrackInstance = aBodyReanim->GetTrackInstanceByName("Zombie_flaghand");
        AttachReanim(aTrackInstance->mAttachmentID, aFlagReanim, 0.0f, 0.0f);
        aBodyReanim->mFrameBasePose = 0;
    ...
    case ZombieType::ZOMBIE_SUPER_HYPNO_FLAG:
    {
        ...
        Reanimation* aFlagReanim2 = mApp->AddReanimation(0.0f, 0.0f, 0, ReanimationType::REANIM_FLAG);
        aFlagReanim2->PlayReanim("Zombie_flag", ReanimLoopType::REANIM_LOOP, 0, 15.0f);
        mSpecialHeadReanimID = mApp->ReanimationGetID(aFlagReanim2);
        ReanimatorTrackInstance* aTrackInstance2 = aBodyReanim2->GetTrackInstanceByName("Zombie_flaghand");
        AttachReanim(aTrackInstance->mAttachmentID, aFlagReanim2, 0.0f, 0.0f);
        aBodyReanim2->mFrameBasePose = 0;
        mBodyHealth = 1850;
        mPosX = WIDE_BOARD_WIDTH;
        ReanimShowPrefix("anim_hair", RENDER_GROUP_HIDDEN);
        break;
    }
    case ZombieType::ZOMBIE_SUPER_NEWSPAPER:
    {
        mHasObject = true;
        LoadPlainZombieReanim();
    ...
    case ZombieType::ZOMBIE_DISCO_GRAVE_BUSTER:
    {
        if (!IsOnBoard())
        {
            PlayZombieReanim("anim_armraise", ReanimLoopType::REANIM_LOOP, 0, 12.0f);
        ...
        ReanimatorTrackInstance* aTrackInstance = aBodyReanim->GetTrackInstanceByName("anim_head1");
        aTrackInstance->mImageOverride = IMAGE_BLANK;
        ReanimatorTrackInstance* aTrackInstance2 = aBodyReanim->GetTrackInstanceByName("Zombie_disco_glasses");
        aTrackInstance2->mImageOverride = IMAGE_BLANK;
        ReanimatorTrackInstance* aTrackInstance3 = aBodyReanim->GetTrackInstanceByName("anim_hair");
        aTrackInstance3->mImageOverride = IMAGE_BLANK;
        ReanimatorTrackInstance* aTrackInstance4 = aBodyReanim->GetTrackInstanceByName("Zombie_disco_chops");
        aTrackInstance4->mImageOverride = IMAGE_BLANK;
        ReanimatorTrackInstance* aTrackInstance5 = aBodyReanim->GetTrackInstanceByName("anim_head2");
        aTrackInstance5->mImageOverride = IMAGE_BLANK;
    ...
    case ZombieType::ZOMBIE_BOSS:  
        ...
        mBodyHealth = mApp->IsAdventureMode() ? 60000 : 90000;
        if (IsOnBoard())
        {
    ...
    case ZombieType::ZOMBIE_GARG_BOSS:
    {
        aRenderLayer = RenderLayer::RENDER_LAYER_TOP;
        mBodyHealth = 50000;
        mWidth = 180;
    ...
    case ZombieType::ZOMBIE_SCAREDY_HEAD:
    {
        LoadPlainZombieReanim();
        ReanimShowPrefix("anim_hair", RENDER_GROUP_HIDDEN);
        ReanimShowPrefix("anim_head2", RENDER_GROUP_HIDDEN);
    ...
    case ZombieType::ZOMBIE_MINE_HEAD:
    {
        LoadPlainZombieReanim();
        ReanimShowPrefix("anim_hair", RENDER_GROUP_HIDDEN);
        ReanimShowPrefix("anim_head2", RENDER_GROUP_HIDDEN);
        ...
        AttachEffect* aAttachEffect = AttachReanim(aTrackInstance->mAttachmentID, aHeadReanim, 0.0f, 0.0f);
        aBodyReanim->mFrameBasePose = 0;
        TodScaleRotateTransformMatrix(aAttachEffect->mOffset, 55.0f, -20.0f, 0.2f, -0.8f, 0.8f);
    ...
    case ZombieType::ZOMBIE_CACTUS_HEAD:
    {
        ...
        aHeadReanim->PlayReanim("anim_idle", ReanimLoopType::REANIM_LOOP, 0, 15.0f);
        mSpecialHeadReanimID = mApp->ReanimationGetID(aHeadReanim);
        AttachEffect* aAttachEffect = AttachReanim(aTrackInstance->mAttachmentID, aHeadReanim, 0.0f, -30.0f);
        aBodyReanim->mFrameBasePose = 0;
        TodScaleRotateTransformMatrix(aAttachEffect->mOffset, 65.0f, -5.0f, 0.2f, -1.0f, 1.0f);
    ...
    case ZombieType::ZOMBIE_CONE_REPEATER_HEAD:
    {
        LoadPlainZombieReanim();
        ReanimShowPrefix("anim_hair", RENDER_GROUP_HIDDEN);
        ReanimShowPrefix("anim_head2", RENDER_GROUP_HIDDEN);
    ...
    case ZombieType::ZOMBIE_ICE_SHROOM_HEAD:
    {
        LoadPlainZombieReanim();
        ReanimShowPrefix("anim_hair", RENDER_GROUP_HIDDEN);
        ReanimShowPrefix("anim_head", RENDER_GROUP_HIDDEN);
        ...
        mBodyHealth = 1500;
        int aDistance = 275 + Rand(175);
        mPhaseCounter = (int)((aDistance / mVelX) * ZOMBIE_LIMP_SPEED_FACTOR)/1.2;
        break;
    }
...
void Zombie::SetupReanimLayers(Reanimation* aReanim, ZombieType theZombieType)
{
    ...
    else if (theZombieType == ZombieType::ZOMBIE_NEWSPAPER || theZombieType == ZombieType::ZOMBIE_SUPER_NEWSPAPER)
    {
        aReanim->AssignRenderGroupToPrefix("Zombie_paper_paper", RENDER_GROUP_HIDDEN);
    }
    else if (theZombieType == ZombieType::ZOMBIE_FLAG || theZombieType == ZombieType::ZOMBIE_RALLY)
    {
...
void Zombie::Mummify()
{
    if (mBoard)
    {
        ...
        if (aBodyReanim->GetTrackInstanceByName("anim_head1")->mImageOverride == nullptr)
        {
        ...
        aBodyReanim->SetImageOverride("Zombie_outerarm_screendoor", IMAGE_BLANK);
        ...
        if (mCamelType == 1)
        {
            aBodyReanim->GetTrackInstanceByName("anim_screendoor")->mImageOverride = IMAGE_REANIM_MUMMY_CAMEL1_1;
        }
        if (mCamelType == 2)
        {
            aBodyReanim->GetTrackInstanceByName("anim_screendoor")->mImageOverride = IMAGE_REANIM_MUMMY_CAMEL2_1;
        }
        if (mCamelType == 3)
        {
            aBodyReanim->GetTrackInstanceByName("anim_screendoor")->mImageOverride = IMAGE_REANIM_MUMMY_CAMEL3_1;
        }
    }
}
...
int Zombie::CountBungeesTargetingSunFlowers()
{
    ...
    {
        if (!aZombie->IsDeadOrDying() && (aZombie->mZombieType == ZombieType::ZOMBIE_BUNGEE || aZombie->mZombieType == ZombieType::ZOMBIE_CHERRY_BUNGEE) && aZombie->mTargetCol != -1)
        {
...
void Zombie::PickRandomSpeed()
{
        ...
        mZombieType == ZombieType::ZOMBIE_POGO || mZombieType == ZombieType::ZOMBIE_FLAG || mZombieType == ZombieType::ZOMBIE_RALLY || mZombieType == ZombieType::ZOMBIE_SUPER_NEWSPAPER)
    {
        if (mZombieType == ZombieType::ZOMBIE_SUPER_NEWSPAPER)
        {
            ...
            {
                 mVelX = RandRangeFloat(3.0f, 3.3f);
            }
        }
        else
        {
            mVelX = 0.6f;
            if (mZombieType == ZombieType::ZOMBIE_RALLY)
            {
                mVelX = 1.2f;
            }
        }
    ...
    {
        mVelX = RandRangeFloat(0.75f, 0.8f);
    }
        ...
        mZombiePhase == ZombiePhase::PHASE_DOLPHIN_WALKING_WITHOUT_DOLPHIN)
    {
        ...
        {
            mVelX = RandRangeFloat(3.0f, 3.2f);
        }
    }
    else if (mZombieType == ZombieType::ZOMBIE_SUPER_JACK)
    {
        mVelX = RandRangeFloat(2.0f, 2.2f);
    }
    else if (mZombieType == ZombieType::ZOMBIE_SUPER_ALLSTAR)
    {
        mVelX = 2.2f;
    }
...
void Zombie::BungeeLiftTarget()
{
    ...
    while (mBoard->IterateZombies(aZombie))
    {
        if ((aZombie->mZombieType == ZombieType::ZOMBIE_BUNGEE || aZombie->mZombieType == ZombieType::ZOMBIE_CHERRY_BUNGEE) && aZombie != this && aZombie->mTargetPlantID == mTargetPlantID)
        {
...
void Zombie::BungeeLanding()
{
    ...
    if (aPlant)
    {
        ...
        aPlant->DoSpecial();
        if (mZombieType == ZombieType::ZOMBIE_CHERRY_BUNGEE)
        {
...
void Zombie::ZombieCatapultFire(Plant* thePlant, int theAttack, int theYOffset)
{
    ...
    if (thePlant)
    {
        ...
        aTargetY = thePlant->mY + theYOffset;
    }
    else
    {
...
void Zombie::UpdateZombieSuperNewspaper()
{
    if (mZombiePhase == ZombiePhase::PHASE_NEWSPAPER_MADDENING)
    {
        Reanimation* aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
...
void Zombie::UpdateZombiePolevaulter()
{
    if (mZombiePhase == ZombiePhase::PHASE_POLEVAULTER_PRE_VAULT && mHasHead && mZombieHeight == ZombieHeight::HEIGHT_ZOMBIE_NORMAL)
    {
        ...
        if (aPlant)
        {
            ...
            if (mZombieType == ZombieType::ZOMBIE_SUPER_POLE_VAULTER)
            {
                SquishAllInSquare(mBoard->PixelToGridXKeepOnBoard(mPosX + 20 , mPosY), mRow, ZombieAttackType::ATTACKTYPE_CHEW);
            }
...
void Zombie::UpdateZombieSuperJack()
{
    if (mZombiePhase == ZombiePhase::PHASE_JACK_IN_THE_BOX_RUNNING)
    {
        if (mPhaseCounter <= 0 && mHasHead)
...
void Zombie::UpdateZombieSuperAllStar()
{
    if (mZombiePhase == ZombiePhase::PHASE_ALLSTAR_RUNNING)
    {
        Plant* aPlant = FindPlantTarget(ZombieAttackType::ATTACKTYPE_CHEW);
        if (aPlant)
        {
            SquishAllInSquare(mBoard->PixelToGridXKeepOnBoard(mPosX-20, mPosY), mRow, ZombieAttackType::ATTACKTYPE_CHEW);
            mZombiePhase = ZombiePhase::PHASE_ALLSTAR_CRUSH;
            mPhaseCounter = 60;
        }
    }
    if (mZombiePhase == ZombiePhase::PHASE_ALLSTAR_CRUSH)
    {
        mPhaseCounter--;
        mVelX = -1.3f;
        UpdateAnimSpeed();
        if (mPhaseCounter <= 1)
        {
            mZombiePhase = ZombiePhase::PHASE_ALLSTAR_WALK;
            mVelX = 1.0f;
            UpdateAnimSpeed();
        }
...
void Zombie::UpdateZombieGargantuarBoss()
{
    bool Idle = false;
    if (mMindControlled && mApp->IsGargBoss())
    {
        mBodyHealth = 0;
    }
    if (mPeaCounter > 0)
    {
        if (!mMindControlled)
        {
            Projectile* aProjectile = mBoard->AddProjectile(mPosX, mPosY, mRenderOrder, mRow, ProjectileType::PROJECTILE_ZOMBIE_HYPNO_PEA);
        ...
        else
        {
            ...
            Projectile* aProjectile = mBoard->AddProjectile(aOriginX, aOriginY, mRenderOrder - 1, mRow, ProjectileType::PROJECTILE_GOO_PEA);
    ...
    if (mZombiePhase == ZombiePhase::PHASE_GARG_BOSS_IDLE)
    {
        if (mPhaseCounter > 0)
        {
            mPhaseCounter--;
        ...
        else
        {
            mApp->PlayFoley(FoleyType::FOLEY_LOW_GROAN);
            PlayZombieReanim("anim_smash", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 20, 16.0f);
            mZombiePhase = ZombiePhase::PHASE_GARG_BOSS_ATTACK;
            bool Idle = true;
        }
        Reanimation* aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
        if (aBodyReanim->IsAnimPlaying("anim_smash") == false && Idle == true)
        {
            StartWalkAnim(20);
            Idle = false;
        }
    ...
    else
    {
        ...
        if (Attack == 2)
        {
            if (!mMindControlled)
            {
                ...
                {
                    aPlant = FindCatapultTargetFront();
                }
                ZombieCatapultFire(aPlant, 2, 0);
                mPhaseCounter = 500;
                if (RandRangeInt(1, 3) == 3)
                {
                    ZombieCatapultFire(aPlant, 2, 80);
                    ZombieCatapultFire(aPlant, 2, -80);
                    ZombieCatapultFire(aPlant, 3, 0);
                    mPhaseCounter = 700;
                }
            }
            else
        ...
        if (Attack == 3)
        {
                ...
                if (!mBoard->RowCanHaveZombieType(mRow, ZombieType::ZOMBIE_SUPER_VASE_HEAD))
                    return;

                Zombie* aZombie = mBoard->AddZombie(ZombieType::ZOMBIE_SUPER_VASE_HEAD, 0);
                if (aZombie == nullptr)
                    return;
                ...
                aZombie->mAltitude = 0;
                aZombie->mPhaseCounter = 150;
                aZombie->mHeight = ZombieHeight::HEIGHT_ZOMBIE_NORMAL;
                aZombie->mBodyHealth = 21;
                aZombie->SwitchLane();


                aZombie = mBoard->AddZombie(ZombieType::ZOMBIE_VASE_HEAD, 0);
                if (aZombie == nullptr)
                    return;

                aZombie->mPosX = mPosX;
                aZombie->SetRow(mRow);
                aPosY = mBoard->GetPosYBasedOnRow(mPosX + 40.0f, mRow) - 30.0f;
                aZombie->mPosY = aPosY;
                aZombie->mX = (int)aZombie->mPosX;
                aZombie->mY = (int)aZombie->mPosY;
                aZombie->PickRandomSpeed();
        ...
        if (Attack == 5)
        {
            Zombie* aBungee1 = mBoard->AddZombie(ZombieType::ZOMBIE_CHERRY_BUNGEE, Zombie::ZOMBIE_WAVE_DEBUG);
            Zombie* aBungee2 = mBoard->AddZombie(ZombieType::ZOMBIE_CHERRY_BUNGEE, Zombie::ZOMBIE_WAVE_DEBUG);
            Zombie* aBungee3 = mBoard->AddZombie(ZombieType::ZOMBIE_CHERRY_BUNGEE, Zombie::ZOMBIE_WAVE_DEBUG);
            if (mMindControlled)
            {
                aBungee1->StartMindControlled();
                aBungee2->StartMindControlled();
                aBungee3->StartMindControlled();
                aBungee1->mPosX += 400;
                aBungee2->mPosX += 400;
                aBungee3->mPosX += 400;
            }
...
void Zombie::ThrowImp(float theDistance, float theScale, float theSpeed)
{
    float aThrowingDistance = mPosX - theDistance;
    Zombie* aZombieImp = mBoard->AddZombie(ZombieType::ZOMBIE_IMP, mFromWave);
    if (aZombieImp == nullptr)
        return;
    ...
    aZombieImp->mRenderOrder = mRenderOrder + 1;
    aZombieImp->mZombiePhase = ZombiePhase::PHASE_IMP_GETTING_THROWN;
    aZombieImp->mScaleZombie = theScale;
    aZombieImp->mBodyHealth *= theScale * theScale;
    aZombieImp->mBodyMaxHealth *= theScale * theScale;

    if (mMindControlled)
    {
        aZombieImp->mPosX = mPosX + mWidth;
        aZombieImp->StartMindControlled();
        aZombieImp->mVelX = -theSpeed;
    }
    else
    {
        aZombieImp->mVelX = theSpeed;
...
void Zombie::UpdateZombieCactusHead()
{
    ...
    else if (mPhaseCounter == 0)
    {
        ...
        Reanimation* aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
        int aTrackIndex = aBodyReanim->FindTrackIndex("anim_idle");
        ReanimatorTransform aTransform;
        aBodyReanim->GetCurrentTransform(aTrackIndex, &aTransform);
        ...
        {
            aOriginX += 90.0f * mScaleZombie;
            Projectile* aProjectile = mBoard->AddProjectile(aOriginX, aOriginY, mRenderOrder, mRow, ProjectileType::PROJECTILE_SPIKE);
            aProjectile->mDamageRangeFlags = 1;
        }
        else
        {
            Projectile* aProjectile = mBoard->AddProjectile(aOriginX, aOriginY, mRenderOrder, mRow, ProjectileType::PROJECTILE_ZOMBIE_SPIKE);
            aProjectile->mMotionType = ProjectileMotion::MOTION_BACKWARDS;
        }
...
void Zombie::UpdateZombieScaredyHead()
{
    if (!mHasHead)
        return;
    ...
    if (mBodyHealth < 600)
    {
        mZombiePhase = ZombiePhase::PHASE_SCAREDY_SCARED;
    }
    else
    {
        mZombiePhase = ZombiePhase::PHASE_ZOMBIE_NORMAL;
    ...
    if (mZombiePhase != ZombiePhase::PHASE_SCAREDY_SCARED)
    {
        if (mPhaseCounter == 35)
        {
            Reanimation* aHeadReanim = mApp->ReanimationGet(mSpecialHeadReanimID);
        ...
        else if (mPhaseCounter == 0)
        {
            ...
            {
                aOriginX += 90.0f * mScaleZombie;
                Projectile* aProjectile = mBoard->AddProjectile(aOriginX, aOriginY, mRenderOrder, mRow, ProjectileType::PROJECTILE_PUFF);
                aProjectile->mDamageRangeFlags = 1;
            }
            else
            {
                Projectile* aProjectile = mBoard->AddProjectile(aOriginX, aOriginY, mRenderOrder, mRow, ProjectileType::PROJECTILE_ZOMBIE_PUFF);
                aProjectile->mMotionType = ProjectileMotion::MOTION_BACKWARDS;
            }
    ...
    {
        Reanimation* aHeadReanim = mApp->ReanimationGet(mSpecialHeadReanimID);
        aHeadReanim->PlayReanim("anim_scaredidle", ReanimLoopType::REANIM_LOOP, 20, 15.0f);
        if (mPhaseCounter == 0)
        {
            mBodyHealth++;
            mPhaseCounter = 3;
...
void Zombie::UpdateZombieBloverHead()
{
    ...
    {
        Reanimation* aHeadReanim = mApp->ReanimationGet(mSpecialHeadReanimID);
        aHeadReanim->PlayReanim("anim_blow", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 20, 35.0f);
    }
    else if (mPhaseCounter == 0)
    {
        Reanimation* aHeadReanim = mApp->ReanimationGet(mSpecialHeadReanimID);
        aHeadReanim->PlayReanim("anim_loop", ReanimLoopType::REANIM_LOOP, 20, 15.0f);
        mApp->PlayFoley(FoleyType::FOLEY_THROW);

        Reanimation* aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
        int aTrackIndex = aBodyReanim->FindTrackIndex("anim_loop");
        ReanimatorTransform aTransform;
        aBodyReanim->GetCurrentTransform(aTrackIndex, &aTransform);
        ...
        else
        {
            ...
            while (mBoard->IteratePlants(aPlant))
            {
                if (!aPlant->IsFirePlant(aPlant->mSeedType) && !aPlant->IsFrozenPlant(aPlant->mSeedType))
                {
                     aPlant->mChilledCounter = 400;
                }
            }
            Zombie* aZombie = nullptr;
            while (mBoard->IterateZombies(aZombie))
...
void Zombie::UpdateZombieMineHead()
{
    if (!mHasHead)
        return;
    ...
    {
        Reanimation* aHeadReanim = mApp->ReanimationGet(mSpecialHeadReanimID);
        aHeadReanim->PlayReanim("anim_rise", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 20, 35.0f);
    }
    else if (mPhaseCounter == 0)
    {
        Reanimation* aHeadReanim = mApp->ReanimationGet(mSpecialHeadReanimID);
        aHeadReanim->PlayReanim("anim_armed", ReanimLoopType::REANIM_LOOP, 20, 15.0f);

        Reanimation* aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
        int aTrackIndex = aBodyReanim->FindTrackIndex("anim_armed");
        ReanimatorTransform aTransform;
        aBodyReanim->GetCurrentTransform(aTrackIndex, &aTransform);
        ...
        if (mHasHead && mIsEating)
        {
            if (!mMindControlled)
            {
                ...
                mBoard->KillAllPlantsInRadius(aPosX, aPosY, 40);
            ...
            else
            {
                ...
                mApp->PlaySample(SOUND_POTATO_MINE);
                mBoard->KillAllZombiesInRadius(mRow, aPosX, aPosY, 60, 0, false, 77, false);
                if (!mApp->IsIZombieLevel() && !mApp->mPlayedQuickplay)
                    mApp->GetAchievement(AchievementType::ACHIEVEMENT_SPUDOW);
...
void Zombie::UpdateZombieConeRepeaterHead()
{
    if (!mHasHead)
        return;

    if (mPhaseCounter == 35 || mPhaseCounter == 65)
    {
        Reanimation* aHeadReanim = mApp->ReanimationGet(mSpecialHeadReanimID);
        aHeadReanim->PlayReanim("anim_shooting", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 20, 35.0f);
    }
    else if (mPhaseCounter == 0 || mPhaseCounter == 45)
    {
        Reanimation* aHeadReanim = mApp->ReanimationGet(mSpecialHeadReanimID);
        aHeadReanim->PlayReanim("anim_head_idle", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 20, 15.0f);
        mApp->PlayFoley(FoleyType::FOLEY_THROW);
...
void Zombie::UpdateZombieHypnoGatlingHead()
{
    if (!mHasHead)
        return;
    ...
    else if (mPhaseCounter == 18 || mPhaseCounter == 35 || mPhaseCounter == 51 || mPhaseCounter == 68)
    {
        ...
        {
            Projectile* aProjectile = mBoard->AddProjectile(aOriginX, aOriginY, mRenderOrder, mRow, ProjectileType::PROJECTILE_ZOMBIE_HYPNO_PEA);
            aProjectile->mMotionType = ProjectileMotion::MOTION_BACKWARDS;
        }
...
ZombieID Zombie::SummonBackupNewspaper(int theRow, int thePosX)
{
    if (!mBoard->RowCanHaveZombieType(theRow, ZombieType::ZOMBIE_NEWSPAPER))
        return ZombieID::ZOMBIEID_NULL;

    Zombie* aZombie = mBoard->AddZombie(ZombieType::ZOMBIE_NEWSPAPER, mFromWave);
    if (aZombie == nullptr)
        return ZombieID::ZOMBIEID_NULL;
    ...
    aZombie->mAltitude = 0;
    aZombie->mZombiePhase = ZombiePhase::PHASE_NEWSPAPER_READING;
    aZombie->mPhaseCounter = 150;
    aZombie->mRelatedZombieID = mBoard->ZombieGetID(this);

    aZombie->SetAnimRate(1.0f);
    aZombie->mMindControlled = mMindControlled;
...
void Zombie::SummonBackupNewspapers()
{
    if (!mHasHead)
        return;
    ...
    for (int i = 0; i < NUM_BACKUP_DANCERS; i++)
    {
        if (mBoard->ZombieTryToGet(mFollowerZombieID[i]) == nullptr)
        {
            ...
            mFollowerZombieID[i] = SummonBackupNewspaper(aRow, aPosX);
        }
    }
}
...
void Zombie::UpdateZombieDancer()
{
    ...
    else if (mZombiePhase == ZombiePhase::PHASE_DANCER_SNAPPING_FINGERS || mZombiePhase == ZombiePhase::PHASE_DANCER_SNAPPING_FINGERS_WITH_LIGHT)
    {
        ...
        if (aBodyReanim->mLoopCount > 0)
        {
            ...
            {
                int aGridX = mBoard->PixelToGridX(mPosX, mPosY) + 1;
                int aGridY = mBoard->PixelToGridYKeepOnBoard(mPosX, mPosY) + 1;
                if (aGridX > 8)
                {
                    aGridX = 8;
                }
                if (aGridX < 8)
                {
                    mBoard->AddAGraveStone(aGridX + 1, aGridY);
                }
                if (aGridX > 0)
                {
                    mBoard->AddAGraveStone(aGridX - 1, aGridY);
                ...
                if (aGridY > 0)
                {
                    mBoard->AddAGraveStone(aGridX, aGridY - 1);
                }
                if (aGridY < 4)
                {
                    mBoard->AddAGraveStone(aGridX, aGridY + 1);
    ...
    else
    {
        ...
        if (mHasHead && mSummonCounter == 0 && NeedsMoreBackupDancers() && mZombieType == ZombieType::ZOMBIE_DANCER)
        {
            mSummonCounter = 100;
        }
    }
...
void Zombie::OverrideParticleColor(TodParticleSystem* aParticle)
{
    if (aParticle)
    {
        ...
        {
            aParticle->OverrideColor(nullptr, Color(161, 52, 235, 255));
            aParticle->OverrideExtraAdditiveDraw(nullptr, true);
        }
        else if (mSunCounter > 0)
        {
            aParticle->OverrideColor(nullptr, Color(255, 242, 0, 200));
            aParticle->OverrideExtraAdditiveDraw(nullptr, true);
        }
    }
}
...
{
    if ((mZombieType != ZombieType::ZOMBIE_FLAG && mZombieType != ZombieType::ZOMBIE_RALLY && mZombieType != ZombieType::ZOMBIE_SUPER_NEWSPAPER) || !mHasObject)
        return;
...
bool Zombie::CanLoseBodyParts()
{
    return 
        ...
        mZombieType != ZombieType::ZOMBIE_CHERRY_BUNGEE &&
        mZombieType != ZombieType::ZOMBIE_GARG_BOSS &&
...
void Zombie::DropHead(unsigned int theDamageFlags)
{
    ...
    else if (mZombieType == ZombieType::ZOMBIE_NEWSPAPER || mZombieType == ZombieType::ZOMBIE_SUPER_NEWSPAPER)
    {
    ...
    else if (mZombieType == ZombieType::ZOMBIE_FLAG || mZombieType == ZombieType::ZOMBIE_RALLY)
    {
        DropFlag();
    }
    ...
    if (aParticle)
    {
        ...
        else if (mZombieType == ZombieType::ZOMBIE_FOOTBALL || mZombieType == ZombieType::ZOMBIE_SUPER_ALLSTAR || mZombieType == ZombieType::ZOMBIE_GIGA_FOOTBALL)
        {
            aParticle->OverrideImage(nullptr, IMAGE_ZOMBIEFOOTBALLHEAD);
        }
        else if (mZombieType == ZombieType::ZOMBIE_POLEVAULTER || mZombieType == ZombieType::ZOMBIE_SUPER_POLE_VAULTER)
        {
...
void Zombie::SetupReanimForLostArm(unsigned int theDamageFlags)
{
    ...
    case ZombieType::ZOMBIE_GIGA_FOOTBALL:
        ReanimShowPrefix("Zombie_football_leftarm_lower", RENDER_GROUP_HIDDEN);
        ReanimShowPrefix("Zombie_black_football_leftarm_hand", RENDER_GROUP_HIDDEN);
        break;
    case ZombieType::ZOMBIE_NEWSPAPER:
    case ZombieType::ZOMBIE_SUPER_NEWSPAPER:
    ...
    if (!mInPool && !TestBit(theDamageFlags, DamageFlags::DAMAGE_DOESNT_LEAVE_BODY))
    {
        ...
        if (aParticle)
        {
            ...
            case ZombieType::ZOMBIE_SUPER_ALLSTAR:
                ...
                break;
            case ZombieType::ZOMBIE_GIGA_FOOTBALL:
                aParticle->OverrideImage(nullptr, IMAGE_REANIM_ZOMBIE_GIGA_FOOTBALL_LEFTARM_HAND);
                break;
            case ZombieType::ZOMBIE_NEWSPAPER:
            case ZombieType::ZOMBIE_SUPER_NEWSPAPER:
...
void Zombie::UpdateZombieWalking()
{
    ...
    if (aBodyReanim)
    {
        ...
        if ((mZombieType == ZombieType::ZOMBIE_FOOTBALL || mZombieType == ZombieType::ZOMBIE_SUPER_ALLSTAR || mZombieType == ZombieType::ZOMBIE_GIGA_FOOTBALL) && mFromWave != Zombie::ZOMBIE_WAVE_WINNER)
        {
    ...
    else
    {
        ...
        if (mZombiePhase == ZombiePhase::PHASE_POLEVAULTER_IN_VAULT || 
            ...
            mZombieType == ZombieType::ZOMBIE_DISCO_GRAVE_BUSTER ||
...
{
    if (mZombieType == ZombieType::ZOMBIE_BUNGEE || mZombieType == ZombieType::ZOMBIE_CHERRY_BUNGEE || mZombieType == ZombieType::ZOMBIE_BOSS ||
...
void Zombie::Update()
{
    ...
    if (doUpdate)
    {
        ...
        else
        {
            ...
            if (mZombieType == ZombieType::ZOMBIE_BUNGEE || mZombieType == ZombieType::ZOMBIE_CHERRY_BUNGEE)
            {
                UpdateZombieBungee();
            }
            if (mZombieType == ZombieType::ZOMBIE_POGO)
...
void Zombie::UpdateActions()
{
    ...
    if (mZombieType == ZombieType::ZOMBIE_POLEVAULTER || mZombieType == ZombieType::ZOMBIE_SUPER_POLE_VAULTER)
    {
    ...
    if (mZombieType == ZombieType::ZOMBIE_SUPER_JACK)
    {
        UpdateZombieSuperJack();
    }
    if (mZombieType == ZombieType::ZOMBIE_SUPER_ALLSTAR)
    {
        UpdateZombieSuperAllStar();
    }
    if (mZombieType == ZombieType::ZOMBIE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR)
    {
        UpdateZombieGargantuar();
    }
    if (mZombieType == ZombieType::ZOMBIE_GARG_BOSS)
    {
        UpdateZombieGargantuarBoss();
    }
    ...
    if (mZombieType == ZombieType::ZOMBIE_SCAREDY_HEAD)
    {
        UpdateZombieScaredyHead();
    }
    if (mZombieType == ZombieType::ZOMBIE_CONE_REPEATER_HEAD)
    {
        UpdateZombieConeRepeaterHead();
    }
    if (mZombieType == ZombieType::ZOMBIE_JALAPENO_HEAD)
    {
        UpdateZombieJalapenoHead();
    }
    if (mZombieType == ZombieType::ZOMBIE_ICE_SHROOM_HEAD)
    {
    ...
    if (mZombieType == ZombieType::ZOMBIE_SUPER_HYPNO_FLAG)
    {
        UpdateZombieHypnoGatlingHead();
    }
    if (mZombieType == ZombieType::ZOMBIE_SQUASH_HEAD)
    {
        UpdateZombieSquashHead();
    }
    if (mZombieType == ZombieType::ZOMBIE_MINE_HEAD)
    {
        UpdateZombieMineHead();
    }
}
...
void Zombie::CheckForBoardEdge()
{
    ...
    if (mZombieType == ZombieType::ZOMBIE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_POLEVAULTER || mZombieType == ZombieType::ZOMBIE_SUPER_POLE_VAULTER)
    {
        aEdgeX = -150;
    }
    else if (mZombieType == ZombieType::ZOMBIE_CATAPULT || mZombieType == ZombieType::ZOMBIE_FOOTBALL || mZombieType == ZombieType::ZOMBIE_SUPER_ALLSTAR || mZombieType == ZombieType::ZOMBIE_GIGA_FOOTBALL || mZombieType == ZombieType::ZOMBIE_ZAMBONI)
    {
        aEdgeX = -175;
    }
...
void Zombie::UpdatePlaying()
{
    ...
    if (mPoisonedCounter > 0 && mBodyHealth > 1)
    {
        mPoisonedCounter--;
        ...
        {
            if ((mZombieType == ZombieType::ZOMBIE_WALLNUT_HEAD || mZombieType == ZombieType::ZOMBIE_TALLNUT_HEAD || mZombieType == ZombieType::ZOMBIE_GIGA_FOOTBALL) && mHelmHealth > 0)
            {
                TakeDamage(1, 3);
            }
            else
...
bool Zombie::HasYuckyFaceImage()
{
    ...
    return 
        ...
        mZombieType == ZombieType::ZOMBIE_SUPER_NEWSPAPER ||
        mZombieType == ZombieType::ZOMBIE_SUPER_POLE_VAULTER||
...
void Zombie::AnimateChewSound()
{
    ...
    if (aPlant)
    {
        ...
        if (aPlant->mSeedType == SeedType::SEED_SHADOW_SHROOM && !aPlant->mIsAsleep)
        {
            mApp->PlayFoley(FoleyType::FOLEY_FLOOP);
            if (aPlant->mShadowPowered > 0) 
            { 
                mBoard->PoisonZombiesRadius(mRow, mPosX, mPosY, 135, 1);
            }
            aPlant->Die();
        ...
        {
            if (aPlant->mSeedType == SeedType::SEED_WALLNUT || aPlant->mSeedType == SeedType::SEED_TALLNUT || aPlant->mSeedType == SeedType::SEED_PUMPKINSHELL || aPlant->mSeedType == SeedType::SEED_INFINUT)
            {
...
void Zombie::Animate()
{
    ...
    if (mIsEating && mHasHead)
    {
        ...
        if (aBodyReanim)
        {
            ...
            if (mZombieType == ZombieType::ZOMBIE_POLEVAULTER || ZombieType::ZOMBIE_SUPER_POLE_VAULTER)
            {
                aLeftHandTime = 0.38f;
                aRightHandTime = 0.8f;
            }
            else if (mZombieType == ZombieType::ZOMBIE_NEWSPAPER || mZombieType == ZombieType::ZOMBIE_SUPER_NEWSPAPER || mZombieType == ZombieType::ZOMBIE_LADDER)
            {
                aLeftHandTime = 0.42f;
                aRightHandTime = 0.42f;
            }
            else if (mZombieType == ZombieType::ZOMBIE_JACK_IN_THE_BOX || mZombieType == ZombieType::ZOMBIE_SUPER_JACK)
            {
                aLeftHandTime = 0.53f;
...
void Zombie::DrawZombiePart(Graphics* g, Image* theImage, int theFrame, int theRow, const ZombieDrawPosition& theDrawPos)
{
    ...
    else if (mPoisonedCounter > 0)
    {
        aMirror = true;
        g->SetColorizeImages(true);
        Color aMincontrolledColor = Color(161, 52, 235, anAlpha);
        aMincontrolledColor.mAlpha = anAlpha;
        g->SetColor(aMincontrolledColor);
        g->DrawImageMirror(theImage, aDestRect, aSrcRect, aMirror);
    ...
    else if (mSunCounter > 0)
    {
        aMirror = true;
        g->SetColorizeImages(true);
        Color aMincontrolledColor = Color(255, 242, 0, anAlpha - 50);
        aMincontrolledColor.mAlpha = anAlpha;
        g->SetColor(aMincontrolledColor);
        g->DrawImageMirror(theImage, aDestRect, aSrcRect, aMirror);
...
void Zombie::UpdateReanim()
{
    ...
    if ((mZombieType == ZombieType::ZOMBIE_FOOTBALL || mZombieType == ZombieType::ZOMBIE_SUPER_ALLSTAR || mZombieType == ZombieType::ZOMBIE_GIGA_FOOTBALL) && mScaleZombie < 1.0f)
    {
    ...
    if (mZombieType == ZombieType::ZOMBIE_DANCER || mZombieType == ZombieType::ZOMBIE_DISCO_GRAVE_BUSTER || mZombieType == ZombieType::ZOMBIE_BACKUP_DANCER)
    {
        anOpposite = false;
...
void Zombie::DrawReanim(Graphics* g, const ZombieDrawPosition& theDrawPos, int theBaseRenderGroup)
{
    ...
    {
        aColorOverride = Color(255, 242, 0, aFadeAlpha - 100);
        aExtraAdditiveColor = aColorOverride;
        aEnableExtraAdditiveDraw = true;
    }
    else if (mPoisonedCounter > 0)
    {
        aColorOverride = Color(161, 52, 235, aFadeAlpha);
        aExtraAdditiveColor = aColorOverride;
        aEnableExtraAdditiveDraw = true;
    }
    ...
    else if (mZombieType == ZombieType::ZOMBIE_SUPER_NEWSPAPER)
    {
        aColorOverride = Color(255, 66, 66, aFadeAlpha);
        aExtraAdditiveColor = aColorOverride;
        aEnableExtraAdditiveDraw = true;
    }
    else if (mZombieType == ZombieType::ZOMBIE_GARG_BOSS)
    {
        aColorOverride = Color(255, 66, 66, aFadeAlpha);
        aExtraAdditiveColor = aColorOverride;
        aEnableExtraAdditiveDraw = true;
    ...
    {
        aColorOverride = Color(66, 93, 245, aFadeAlpha);
        aExtraAdditiveColor = aColorOverride;
        aEnableExtraAdditiveDraw = true;
    }
    ...
    if (mZombieType == ZombieType::ZOMBIE_SCAREDY_HEAD)
    {
        aColorOverride = Color(121, 68, 115, aFadeAlpha);
        aExtraAdditiveColor = aColorOverride;
        aEnableExtraAdditiveDraw = true;
        Reanimation* aHeadReanim = mApp->ReanimationGet(mSpecialHeadReanimID);
    ...
    else if (mZombieType == ZombieType::ZOMBIE_BUNGEE || mZombieType == ZombieType::ZOMBIE_CHERRY_BUNGEE)
    {
...
bool Zombie::CanTargetPlant(Plant* thePlant, ZombieAttackType theAttackType)
{
    ...
    if (mZombiePhase == ZombiePhase::PHASE_LADDER_CARRYING || mZombiePhase == ZombiePhase::PHASE_LADDER_PLACING)
    {
        ...
        if (thePlant->mSeedType == SeedType::SEED_WALLNUT || thePlant->mSeedType == SeedType::SEED_TALLNUT || thePlant->mSeedType == SeedType::SEED_PUMPKINSHELL || thePlant->mSeedType == SeedType::SEED_INFINUT || thePlant->mSeedType == SeedType::SEED_UMBRELLA)
        {
            aPlaceLadder = true;
        }
...
{
    if ((mChilledCounter > 0) || (mHoneyPuddle > 0) || (mPoisonedCounter > 0 && mPoisonedType))
        return true;
...
void Zombie::ApplyAnimRate(float theAnimRate)
{
    ...
    if (aBodyReanim)
    {
        ...
        aBodyReanim->mAnimRate =  theAnimRate * aSpeedMult;
    }
}
...
void Zombie::UpdateAnimSpeed()
{
    ...
    if (mIsEating)
    {
        if (mZombieType == ZombieType::ZOMBIE_POLEVAULTER || ZombieType::ZOMBIE_SUPER_POLE_VAULTER|| mZombieType == ZombieType::ZOMBIE_BALLOON || mZombieType == ZombieType::ZOMBIE_IMP ||
...
{
    if (mZombieHeight != ZombieHeight::HEIGHT_ZOMBIE_NORMAL || mZombieType == ZombieType::ZOMBIE_BUNGEE || mZombieType == ZombieType::ZOMBIE_CHERRY_BUNGEE)
        return;
...
void Zombie::EatPlant(Plant* thePlant)
{
    ...
    if (thePlant->mSeedType == SeedType::SEED_UMBRELLA && thePlant->mLaunchCounter <= 0)
    {
        thePlant->DoSpecial();
        thePlant->mPlantHealth -= DAMAGE_PER_EAT;
    }
    if (mVelX >= 1.0f)
    {
        thePlant->mPlantHealth -= (DAMAGE_PER_EAT * (mVelX - 1.0f)) / 1.5;
    }
    ...
    if (thePlant->mPlantHealth <= 0 && thePlant->mSeedType != SeedType::SEED_INFINUT)
    {
    ...
    else if (thePlant->mPlantHealth <= 0 && thePlant->mSeedType == SeedType::SEED_INFINUT)
    {
        thePlant->mLaunchCounter = 1500;
    }
}
...
bool Zombie::TrySpawnLevelAward()
{
    ...
    else if (mApp->IsAdventureMode() && mBoard->mLevel <= 70)
    {
        ...
        else if (mBoard->mLevel == NUM_LEVELS)
        {
    ...
    aCoinMotion = CoinMotion::COIN_MOTION_COIN;
    if (mZombieType == ZombieType::ZOMBIE_BOSS)
    {
...
void Zombie::DieNoLoot()
{
    if (mSunCounter > 0)
    {
        mBoard->AddCoin(mPosX, mPosY, COIN_SMALLSUN, COIN_MOTION_COIN);
    }
    StopZombieSound();
...
void Zombie::StopZombieSound()
{
        ...
        if (mBoard)
        {
            ...
            while (mBoard->IterateZombies(aZombie))
            {
                if (aZombie->mHasHead && !aZombie->IsDeadOrDying() && aZombie->IsOnBoard() && 
                    (aZombie->mZombieType == ZombieType::ZOMBIE_DANCER || aZombie->mZombieType == ZombieType::ZOMBIE_DISCO_GRAVE_BUSTER || aZombie->mZombieType == ZombieType::ZOMBIE_BACKUP_DANCER))
                {
                    aStopSound = true;
                    break;
                }
    ...
    if (mPlayingSong)
    {
        ...
        if (mZombieType == ZombieType::ZOMBIE_JACK_IN_THE_BOX || mZombieType == ZombieType::ZOMBIE_SUPER_JACK)
        {
...
void Zombie::DropShield(unsigned int theDamageFlags)
{
    ...
    if (mShieldType == ShieldType::SHIELDTYPE_DOOR)
    {
        ...
        if (!TestBit(theDamageFlags, (int)DamageFlags::DAMAGE_DOESNT_LEAVE_BODY))
        {
            ...
            if (mCamelType == 1 && mIsMummy)
            {
                aParticle->OverrideImage(nullptr, IMAGE_REANIM_MUMMY_CAMEL1_3);
            }
            if (mCamelType == 2)
            {
                aParticle->OverrideImage(nullptr, IMAGE_REANIM_MUMMY_CAMEL2_3);
...
int Zombie::TakeShieldDamage(int theDamage, unsigned int theDamageFlags)
{
    ...
    if (aDamageIndexAfterDamage != aDamageIndexBeforeDamage)
    {
        ...
        if (mShieldType == ShieldType::SHIELDTYPE_DOOR && aDamageIndexAfterDamage == 1)
        {
            ...
            if (mCamelType == 1 && mIsMummy)
            {
                aBodyReanim->SetImageOverride("anim_screendoor", IMAGE_REANIM_MUMMY_CAMEL1_2);
            }
            if (mCamelType == 2)
            {
                aBodyReanim->SetImageOverride("anim_screendoor", IMAGE_REANIM_MUMMY_CAMEL2_2);
            }
            if (mCamelType == 3)
            {
                aBodyReanim->SetImageOverride("anim_screendoor", IMAGE_REANIM_MUMMY_CAMEL3_2);
            }
        }
        else if (mShieldType == ShieldType::SHIELDTYPE_DOOR && aDamageIndexAfterDamage == 2)
        {
            ...
            if (mCamelType == 1 && mIsMummy)
            {
                aBodyReanim->SetImageOverride("anim_screendoor", IMAGE_REANIM_MUMMY_CAMEL1_3);
            }
            if (mCamelType == 2)
            {
                aBodyReanim->SetImageOverride("anim_screendoor", IMAGE_REANIM_MUMMY_CAMEL2_3);
            }
            if (mCamelType == 3)
            {
                aBodyReanim->SetImageOverride("anim_screendoor", IMAGE_REANIM_MUMMY_CAMEL3_3);
            }
        }
...
void Zombie::DropHelm(unsigned int theDamageFlags)
{
    ...
    {
        GetTrackPosition("zombie_black_football_helmet", aPosX, aPosY);
        ReanimShowPrefix("zombie_black_football_helmet", RENDER_GROUP_HIDDEN);
        ReanimShowPrefix("anim_hair", RENDER_GROUP_NORMAL);
        aEffect = ParticleEffect::PARTICLE_ZOMBIE_HELMET;
    }
    ...
    {
        TodParticleSystem* aParticle = mApp->AddTodParticle(aPosX, aPosY, mRenderOrder + 1, aEffect);
        if (mIsMummy&& aEffect == ParticleEffect::PARTICLE_ZOMBIE_TRAFFIC_CONE)
        {
            aParticle->OverrideImage(nullptr, IMAGE_REANIM_MUMMY_CONE3);
        }
        if (mIsMummy && aEffect == ParticleEffect::PARTICLE_ZOMBIE_PAIL)
        {
            aParticle->OverrideImage(nullptr, IMAGE_REANIM_MUMMY_BUCKET3);
        }
        OverrideParticleScale(aParticle);
    }
...
void Zombie::TakeDamage(int theDamage, unsigned int theDamageFlags)
{
    ...
    if ((mZombieType == ZombieType::ZOMBIE_VASE_HEAD || mZombieType == ZombieType::ZOMBIE_SUPER_VASE_HEAD) && mBodyHealth < 20)
    {
        UpdateZombieVaseHead();
    }
}
...
{
    if (mZombieType == ZombieType::ZOMBIE_ZAMBONI || mZombieType == ZombieType::ZOMBIE_GARG_BOSS || IsBobsledTeamWithSled())
        return false;
...
bool Zombie::CanBeFrozen()
{
    ...
    return (mZombieType != ZombieType::ZOMBIE_BUNGEE && mZombieType != ZombieType::ZOMBIE_CHERRY_BUNGEE)|| mZombiePhase == ZombiePhase::PHASE_BUNGEE_AT_BOTTOM;
}
...
bool Zombie::EffectedByDamage(unsigned int theDamageRangeFlags)
{
    ...
    if ((mZombieType == ZombieType::ZOMBIE_BUNGEE || mZombieType == ZombieType::ZOMBIE_CHERRY_BUNGEE) && mZombiePhase != ZombiePhase::PHASE_BUNGEE_AT_BOTTOM && mZombiePhase != ZombiePhase::PHASE_BUNGEE_GRABBING)
    {
        return false;  
    }
...
bool Zombie::IsZombotany(ZombieType theZombieType)
{
    return
        ...
        theZombieType == ZombieType::ZOMBIE_CHERRY_BUNGEE ||
        theZombieType == ZombieType::ZOMBIE_MINE_HEAD ||
        theZombieType == ZombieType::ZOMBIE_CACTUS_HEAD ||
        theZombieType == ZombieType::ZOMBIE_CONE_REPEATER_HEAD ||
        theZombieType == ZombieType::ZOMBIE_SUPER_HYPNO_FLAG ||
        theZombieType == ZombieType::ZOMBIE_MELON_PULT_HEAD ||
        theZombieType == ZombieType::ZOMBIE_BUTTER_PULT_HEAD ||
        theZombieType == ZombieType::ZOMBIE_SCAREDY_HEAD ||
        theZombieType == ZombieType::ZOMBIE_BLOVER_HEAD ||
        theZombieType == ZombieType::ZOMBIE_DISCO_GRAVE_BUSTER ||
        theZombieType == ZombieType::ZOMBIE_WALLNUT_HEAD ||
        theZombieType == ZombieType::ZOMBIE_TALLNUT_HEAD || 
        theZombieType == ZombieType::ZOMBIE_ICE_SHROOM_HEAD ||
...
bool Zombie::ZombieTypeCanGoInPool(ZombieType theZombieType)
{
    return 
        ...
        theZombieType == ZombieType::ZOMBIE_RALLY || 
...
void Zombie::HitIceTrap()
{
    ...
    mIceTrapCounter = max(aIceTrapCounter, mIceTrapCounter);
...
void Zombie::HitIceCabbage()
{
    ...
    {
        aIceTrapCounter = RandRangeInt(10, 15);
    }
    else
    {
        aIceTrapCounter = RandRangeInt(5, 15);
    }
    mIceTrapCounter = max(aIceTrapCounter, mIceTrapCounter);
...
void Zombie::HitFrostBolt()
{
    ...
    {
        aIceTrapCounter = RandRangeInt(30, 40);
    }
    else
    {
        aIceTrapCounter = RandRangeInt(35, 55);
    }
    mIceTrapCounter = max(aIceTrapCounter, mIceTrapCounter);
    StopZombieSound();
    if (mZombieType == ZombieType::ZOMBIE_BALLOON)
...
void Zombie::RemoveButter()
{
    ...
    if (Zombie::IsZombotany(mZombieType))
    {
        ...
        if (aHeadReanim)
        {
            ...
            if (mZombieType == ZombieType::ZOMBIE_CACTUS_HEAD && aHeadReanim->IsAnimPlaying("anim_shooting"))
            {
                aHeadReanim->mAnimRate = 35.0f;
            }
            if (mZombieType == ZombieType::ZOMBIE_SNOW_PEA_HEAD && aHeadReanim->IsAnimPlaying("anim_shooting"))
            {
                aHeadReanim->mAnimRate = 35.0f;
            }
...
void Zombie::MowDown()
{
    if (mDead || mZombiePhase == ZombiePhase::PHASE_ZOMBIE_MOWERED || mZombieType == ZombieType::ZOMBIE_BOSS || mZombieType == ZombieType::ZOMBIE_GARG_BOSS)
        return;
    ...
    else if (mZombieType == ZombieType::ZOMBIE_NEWSPAPER || mZombieType == ZombieType::ZOMBIE_BALLOON || mZombieType == ZombieType::ZOMBIE_SUPER_NEWSPAPER)
    {
        DropHead(0U);
    }
    else if (mZombieType == ZombieType::ZOMBIE_POGO)
...
void Zombie::ApplyBurn()
{
    ...
    else if (mZombieType == ZOMBIE_BUNGEE || mZombieType == ZOMBIE_CHERRY_BUNGEE || mZombieType == ZOMBIE_YETI || Zombie::IsZombotany(mZombieType) || IsBobsledTeamWithSled() || IsFlying() || !mHasHead)
    {
        SetAnimRate(0.0f);
        Reanimation* aHeadReanim = mApp->ReanimationTryToGet(mSpecialHeadReanimID);
        if (aHeadReanim)
...
void Zombie::PlayDeathAnim(unsigned int theDamageFlags)
{
    ...
    if (mZombieType == ZombieType::ZOMBIE_FOOTBALL || mZombieType == ZombieType::ZOMBIE_SUPER_ALLSTAR || mZombieType == ZombieType::ZOMBIE_GIGA_FOOTBALL)
    {
...
void Zombie::DoDaisies()
{
    ...
    if (mZombieType == ZombieType::ZOMBIE_FOOTBALL || mZombieType == ZombieType::ZOMBIE_SUPER_ALLSTAR || mZombieType == ZombieType::ZOMBIE_GIGA_FOOTBALL || mZombieType == ZombieType::ZOMBIE_DANCER || mZombieType == ZombieType::ZOMBIE_DISCO_GRAVE_BUSTER || mZombieType == ZombieType::ZOMBIE_BACKUP_DANCER)
    {
        aOffsetX += 160.0f;
    }
    else if (mZombieType == ZombieType::ZOMBIE_POGO)
...
void Zombie::DrawShadow(Graphics* g)
{
    ...
    if (mZombieType == ZombieType::ZOMBIE_NEWSPAPER || mZombieType == ZombieType::ZOMBIE_SUPER_NEWSPAPER)
    {
        aShadowOffsetY += 4.0f;
    }
    else if (mZombieType == ZombieType::ZOMBIE_BALLOON)
    ...
    if (mZombieHeight == ZombieHeight::HEIGHT_UP_LADDER || mZombieHeight == ZombieHeight::HEIGHT_FALLING || 
        mZombiePhase == ZombiePhase::PHASE_IMP_GETTING_THROWN || mZombieType == ZombieType::ZOMBIE_BUNGEE || mZombieType == ZombieType::ZOMBIE_CHERRY_BUNGEE || IsBouncingPogo() || IsFlying())
    {
...
void Zombie::WalkIntoHouse()
{
    ...
    else if (mBoard->mBackground == BackgroundType::BACKGROUND_5_ROOF || mBoard->mBackground == BackgroundType::BACKGROUND_6_BOSS)
    {
        ...
        Reanimation* aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
        if (aBodyReanim && aBodyReanim->TrackExists("anim_idle") && mZombieType != ZombieType::ZOMBIE_POLEVAULTER && mZombieType != ZombieType::ZOMBIE_SUPER_POLE_VAULTER)
        {
